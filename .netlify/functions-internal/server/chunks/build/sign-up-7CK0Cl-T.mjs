...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><h2 class="mt-6 text-3xl font-bold text-gray-900 dark:text-white">${ssrInterpolate(_ctx.$t("auth.createAccount") || "Create your account")}</h2><p class="mt-2 text-sm text-gray-600 dark:text-gray-400">${ssrInterpolate(_ctx.$t("app.description"))}</p></div><div class="flex justify-center">`);
      _push(ssrRenderComponent(_component_ClientOnly, null, {
        fallback: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(_component_AuthFormSkeleton, { "is-sign-up": true }, null, _parent2, _scopeId));
          } else {
            return [
              createVNode(_component_AuthFormSkeleton, { "is-sign-up": true })
            ];
          }
        })
      }, _parent));
      _push(`</div><div class="text-center"><p class="text-sm text-gray-600 dark:text-gray-400">${ssrInterpolate(_ctx.$t("auth.hasAccount") || "Already have an account?")} `);
